...
### Destination projection gap (ncl handlers)

The `ncl destinations add/update/remove` handlers mutate `agent_destinations` in the central DB via raw SQL, but never call `writeDestinations()` to propagate the change to running containers. The `agent-destinations.ts` module documents this invariant:

> every time you call `createDestination` / `deleteDestination` from code that runs while an agent's container may be alive, you MUST also call `writeDestinations(agentGroupId, sessionId)` for each affected session.

Without this projection, destination edits via `ncl` don't take effect until the next container restart. This matters for topic routing because setting `thread_id` on a destination via `ncl destinations update --thread-id` won't be visible to the container until it restarts — the `send_message({ to: "alerts" })` call will fail with "unknown destination" or use stale data.

**Fix:** `src/cli/resources/destinations.ts` — import `getSessionsByAgentGroup` and dynamically import `writeDestinations`, then call `projectToSessions(agentGroupId)` after each mutation. The dynamic import is guarded by a try/catch so the handler degrades gracefully when the agent-to-agent module isn't installed.
